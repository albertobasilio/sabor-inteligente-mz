...
const ShoppingListPage = () => {
    ...
    const [newItem, setNewItem] = useState({ item_name: '', quantity: '', unit: '' });
        ...
        <div>
                        ...
                        <>
                            {newListItems.map(item => (
                                <div className="shopping-item" key={item.id}>
                                    ...
                                    <button onClick={() => removeItem(item.id)} style={{ background: 'none', border: 'none', color: '#ff4757', cursor: 'pointer', fontSize: '1.1rem' }}>
                                        x
                                    </button>
                                </div>
                            ))}

                            <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border-color)' }}>
            ...
            {lists.length > 0 ? (
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                            <div>
                                ...
                                <div style={{ display: 'flex', gap: 16, fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: 4 }}>
                                    ...
                                    <span>{(list.items || []).filter(i => i.is_purchased).length}/{(list.items || []).length} concluidos</span>
                                </div>
                            </div>
                ...
                <div className="empty-state">
                    ...
                    <p>Crie uma lista para organizar suas compras.</p>
